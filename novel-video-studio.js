(() => {
  ...
  async function renderPdfPageForVision(page){const base=page.getViewport({scale:1});const scale=Math.min(3,1800/Math.max(base.width,base.height));const viewport=page.getViewport({scale:Math.max(1.8,scale)});const pageCanvas=document.createElement("canvas");pageCanvas.width=Math.ceil(viewport.width);pageCanvas.height=Math.ceil(viewport.height);const pageContext=pageCanvas.getContext("2d",{alpha:false});pageContext.fillStyle="#fff";pageContext.fillRect(0,0,pageCanvas.width,pageCanvas.height);await page.render({canvasContext:pageContext,viewport}).promise;return pageCanvas.toDataURL("image/jpeg",.84);}
  async function pdfToVoice(){if(!lastPdfDocument)return toast("กรุณาอัปโหลด PDF ก่อน");const auth=session();if(!auth?.token)return toast("กรุณาล็อกอินก่อนใช้ AI อ่าน PDF");const total=lastPdfDocument.numPages;if(total>40&&!confirm(`PDF มี ${total} หน้า การอ่านและสร้างเสียงอาจใช้เวลานาน ต้องการทำต่อหรือไม่?`))return;const button=$("pdfToVoiceBtn");button.disabled=true;$("ocrPdfBtn").disabled=true;button.innerHTML='<i data-lucide="loader-circle" class="spin"></i> กำลังเตรียม PDF...';icons();try{const pages=[];for(let i=1;i<=total;i++){button.textContent=`AI อ่านหน้า ${i}/${total}`;$("repairStatus").textContent=`กำลังอ่านภาพหน้า ${i}/${total} โดยตรง ไม่ใช้ OCR`;const image=await renderPdfPageForVision(await lastPdfDocument.getPage(i));const response=await fetch(`${apiUrl}/api/video/pdf-vision`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${auth.token}`},body:JSON.stringify({image,page:i,total})});const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||`อ่านหน้า ${i} ไม่สำเร็จ`);pages.push(String(data.text||"").trim());}const text=cleanText(pages.filter(Boolean).join("\n\n"));if(!text)throw new Error("AI อ่านข้อความจาก PDF ไม่ได้");$("manuscript").value=text;parseManuscript(text);$("repairStatus").textContent="AI อ่านภาพ PDF ครบแล้ว กำลังสร้างเสียงพากย์";await createVoiceFromText(text,button,"สร้างเสียง");$("repairStatus").textContent="PDF ถูกอ่านจากภาพและสร้างเสียงพากย์เรียบร้อย";toast("PDF → เสียงพากย์ พร้อมเล่นแล้ว");}catch(error){toast(`PDF → เสียงไม่สำเร็จ: ${error.message}`);$("repairStatus").textContent="ทำรายการไม่สำเร็จ กรุณาลองใหม่หรือลดจำนวนหน้า";}finally{button.disabled=false;$("ocrPdfBtn").disabled=false;button.innerHTML='<i data-lucide="headphones"></i> PDF → เสียงทันที';icons();}}
  ...
  async function readManuscript(file){if(!file)return;if(file.size>15*1024*1024)return toast("ไฟล์ใหญ่เกิน 15 MB");$("saveState").textContent="กำลังอ่านไฟล์...";try{const ext=file.name.split(".").pop().toLowerCase();let text="";const buffer=await file.arrayBuffer();lastPdfDocument=null;ocrReviewPages=[];$("pdfToVoiceBtn").disabled=true;$("ocrPdfBtn").disabled=true;$("reviewOcrBtn").disabled=true;if(ext==="txt")text=decodeTextFile(buffer);else if(ext==="docx"){if(!window.mammoth)throw new Error("โหลดตัวอ่าน DOCX ไม่สำเร็จ");text=(await mammoth.extractRawText({arrayBuffer:buffer})).value;}else if(ext==="pdf"){if(!window.pdfjsLib)throw new Error("โหลดตัวอ่าน PDF ไม่สำเร็จ");pdfjsLib.GlobalWorkerOptions.workerSrc="https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js";lastPdfDocument=await openPdf(new Uint8Array(buffer));$("pdfToVoiceBtn").disabled=false;$("ocrPdfBtn").disabled=false;$("manuscript").value="";parseManuscript("");$("repairStatus").textContent=`PDF พร้อม ${lastPdfDocument.numPages} หน้า เลือก PDF → เสียงทันที หรือแปลงเป็นข้อความ`;toast(`โหลด ${file.name} แล้ว กด PDF → เสียงทันทีได้เลย`);return;}else throw new Error("รองรับเฉพาะ TXT, DOCX และ PDF");const repaired=repairMojibake(text);$("manuscript").value=repaired;parseManuscript(repaired);$("repairStatus").textContent=repaired!==text?"ระบบซ่อม encoding ให้อัตโนมัติแล้ว":"อ่านไฟล์และตรวจ encoding แล้ว";toast(`อ่าน ${file.name} สำเร็จ${repaired!==text?" และซ่อมข้อความแล้ว":""}`);}catch(error){toast(error.message);}finally{$("saveState").textContent="บันทึกร่างแล้ว";}}
  ...
  $("pdfToVoiceBtn").addEventListener("click",pdfToVoice);
